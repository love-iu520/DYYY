...
generate_build_run_title() {
    ...
    local joined=""
    local title
    local max_title_length=${BUILD_RUN_TITLE_MAX_LENGTH:-240}
    local trim_length
    local summaries=()
    ...
    while IFS=$'\t' read -r hash subject commit_type; do
        ...
        summary_title=${summary_title/#修正适配/适配}
        summaries+=("$summary_title")
    done < "$records_file"
    ...
    for summary_title in "${summaries[@]}"; do
        if [[ -n "$joined" ]]; then
            joined+="；"
        fi
        joined+="$summary_title"
    done

    title="构建 ${#summaries[@]} 项变更：${joined}"
    if (( ${#title} > max_title_length )); then
        trim_length=$((max_title_length - 1))
        title="${title:0:trim_length}…"
    fi

    printf '%s\n' "$title"
}

if [[ "${1:-}" == "--build-title" ]]; then
    if [[ "$#" -ne 3 ]]; then
        echo "Usage: $0 --build-title <before> <after>" >&2
        exit 2
    fi

    generate_build_run_title "$2" "$3"
    exit 0
fi
