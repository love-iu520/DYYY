...
- (void)viewDidAppear:(BOOL)animated {
    %orig;
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(checkAndSkipLowLikes) name:@"DYYYCheckNextAwemeLikes" object:nil];
}
...
- (void)checkAndSkipLowLikes {
    ...
    if (tableView && [tableView isKindOfClass:[UITableView class]]) {
        ...
        while (nextIndex < dataArray.count) {
            ...
            id statistics = [nextAweme valueForKey:@"statistics"];
            if (statistics && [statistics respondsToSelector:@selector(valueForKey:)]) {
                NSNumber *diggNumber = [statistics valueForKey:@"diggCount"];
                if ([diggNumber isKindOfClass:[NSNumber class]]) {
                    diggCount = [diggNumber integerValue];
                }
            }
...
- (void)dealloc {
    [[NSNotificationCenter defaultCenter] removeObserver:self name:@"DYYYCheckNextAwemeLikes" object:nil];
    %orig;
}
